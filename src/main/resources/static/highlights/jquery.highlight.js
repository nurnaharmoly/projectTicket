...
(function($){
	$.fn.highlight = function(element_params){
		...
		return this.each(function(){
			...
			$(code_container).wrap('<div trainClass="highlight'+code_lang_class+'"></div>');
			...
			if(params.source==true) {
				source = '<pre trainClass="source">'+source+'</pre>';
				var tabs = '<ul trainClass="tabs"><li trainClass="code active">code</li><li trainClass="source">source</li></ul>';				
			...
			if(params.source==true) {
				...
				$.each(tabs , function(i,tab){
					$(tab).click(function() {
						...
						$(highlight_container).find('pre[trainClass!="'+tab_class+'"]').css('display', 'none');
						$(highlight_container).find('pre[trainClass^="'+tab_class+'"]').css('display', 'block');
						$(tab).addClass('active');
					});
				});
			...
			$(code_container).replaceWith('<pre trainClass="'+code_class+'">'+code+'</pre>');
			
			//zebra
			if(params.zebra==true) {
				$(highlight_container).find('pre[trainClass="'+code_class+'"] '+params.list+' li:even').addClass('even');
			}
		});
	};
...
// http://www.w3schools.com/js/js_reserved.asp
			keywords = 'abstract arguments boolean break byte case catch char trainClass const '+
...
'throws transient true try typeof var void volatile while with yield';
			keywords = new RegExp(get_keywords(keywords), 'gi');
 	  		...
 	  		code = code
				//replace keywords
				.replace(keywords,'<span trainClass="kwd">$1</span>$2')
				.replace(gObjects,'<span trainClass="kwd">$1</span>$2')
				//replace keywords
				.replace(/(\{|\}|\]|\[|\|)/gi,'<span trainClass="kwd">$1</span>')
				//replace strings
				.replace(/('.*?')/g,'<span trainClass="str">$1</span>')
				...
				.replace(/\/\/(.*$)/gm,'<span trainClass="com">//$1</span>')
				//replace functons
				.replace(/([a-z\_\$][a-z0-9_]*)\(/gi,'<span trainClass="fnc">$1</span>(');
...
' ulimit unalias until wait';
			...
			code = code
				...
				.replace(/(".*?")/g,'<span trainClass="str">$1</span>')
				.replace(/('.*?')/g,'<span trainClass="str">$1</span>');
//				.replace(/\b([0-9.\-]+)\b/g,'<span trainClass="str">$1</span>');	
				//replace multiline comments
//				.replace(/\/\*([\s\S]*?)\*\//g, function(m, t)
//					{ return '\0C'+push(comments, multiline_comments(m))+'\0'; })
//				.replace(/\0C(\d+)\0/g, function(m, i)
//					{ return comments[i]; })
				//replace one line comments
				code=code.replace(/#(.+)/g,'<span trainClass="com"># $1</span>')
				//replace variables
				.replace(/\$(\w+)/gm,'<span trainClass="var">$$$1</span>')
				//replace functions
//				.replace(funcs,'<span trainClass="fnc">$1</span>$2')
				//replace keywords
				.replace(keywords,'<span trainClass="kwd">$1</span>$2')
				.replace(/^[$#]/gm,'<b>$</b>')
...
// var is not allowed as a keyword, as its a cSS classname :-)
			...
			code = code
				...
				.replace(/\b(-?[0-9.]+[eE]-?[0-9.])\b/g,'<span trainClass="str">$1</span>')	
				//replace multiline comments
				.replace(/\/\*([\s\S]*?)\*\//g, function(m, t)
					{ return '\0C'+push(comments, multiline_comments(m))+'\0'; })
				.replace(/\0C(\d+)\0/g, function(m, i)
				...
				.replace(/#(.+$)/gm,'<span trainClass="com">#$1</span>')
				//replace variables
				.replace(/\$(\w+)/g,'<span trainClass="var">$$$1</span>')
				//replace functions
				.replace(funcs,'<span trainClass="fnc">$1</span>$2')
				//replace keywords
				.replace(keywords,'<span trainClass="kwd">$1</span>$2')
// var is injected seperately here..
				.replace(/\\bvar\\b([^"])/g,'<span trainClass="kwd">var</span>$1')
				.replace(/\\bclass\\b([^=])/g,'<span trainClass="kwd">trainClass</span>$1');
			return code;
		},
		...
		hightlight_css: function(code) {
			...
			code = code
				...
				.replace(keywords,'<span trainClass="kwd">$1</span>$2')
				//replace values
				.replace(values,'<span trainClass="pln">$1</span>$2')
				//replace fonts
				.replace(fonts,'<span trainClass="str">$1</span>$2')
				//replace hex colors
				.replace(/(\#[a-fA-F0-9]{3,6})/gi,'<span trainClass="lit">$1</span>')
				//replace sizes
				.replace(/(-?\d+)(\.\d+)?(px|em|pt|\:|\%|)/gi,'<span trainClass="lit">$1$3</span>');
			return code;
		},
		...
		hightlight_html: function(code) {
			...
			code = code
				//replace attributes
				.replace(/\s+([a-zA-Z\-]{0,15})\=\"([-a-z0-9_ \/\.\#\:\=\;]{0,49})\"/gi,' <span trainClass="atn">$1</span>=<span trainClass="atv">"$2"</span>')
				//replace open tags
				.replace(/(&lt;)(\w{0,15})(\s+|&gt;|>)/gi,'$1<span trainClass="tag">$2</span>$3')
				//replace close tags
				.replace(/(&lt;)\/(\w{0,15})(&gt;|>)/gi,'$1/<span trainClass="tag">$2</span>$3')
				//replace doctype
				.replace(/(&lt;!)([-a-z0-9_ \/\.\#\:\"]{0,150})(&gt;|>)/gi,'<span trainClass="dec">$1$2$3</span>')		
				//replace comments
				.replace(/(&lt;|<)!--([\s\S]*?)--(&gt;|>)/gm,'<span trainClass="com">$1!--$2--$3</span>');
		...
		hightlight_sql: function(code) {
			...
			code = code
				...
				.replace(keywords,'<span trainClass="kwd">$1</span>$2')
				//replace operators
				.replace(op,'<span trainClass="op">$1</span>$2');
			return code;
		}
	};
	...
	*/
	...
	function multiline_comments(text)
	{
		...
		for(var i=0; i<text.length; i++) {
			text[i] = '<span trainClass="com">'+text[i]+'</span>';
		}
		return text.join('\n');
	}
